...
func GetExtRecharge(v Setup) (eRch []ExtRch, err error) {
	query := fmt.Sprintf("select yr, mnth, file_type, rch, node from ext_recharge inner join model_cells mc "+
		"on st_contains(mc.geom, ext_recharge.geom) where yr >= %d and yr <= %d ;", v.SYear, v.EYear)
